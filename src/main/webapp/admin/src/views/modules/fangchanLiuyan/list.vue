		...
		<div v-if="showFlag">
			<el-form :inline="true" :model="searchForm" class="form-content">
				<el-row :gutter="20" class="slt"
					...
					<el-form-item :label="contents.inputTitle == 1 ? '新房名称' : ''">
						<el-input prefix-icon="el-icon-search" v-model="searchForm.fangchanName" placeholder="新房名称"
							clearable></el-input>
					</el-form-item>
				...
				<el-row class="ad"
					...
					<el-form-item>
						<el-button v-if="isAuth('fangchanLiuyan','新增')" type="success" icon="el-icon-plus"
							@click="addOrUpdateHandler()">新增</el-button>
						&nbsp;
						<el-button v-if="isAuth('fangchanLiuyan','删除')" :disabled="dataListSelections.length <= 0"
							type="danger" icon="el-icon-delete" @click="deleteHandler()">删除</el-button>
						&nbsp;
						<el-button v-if="isAuth('fangchanLiuyan','报表')" type="success" icon="el-icon-pie-chart"
							@click="chartDialog()">报表</el-button>
						&nbsp;
						<a style="text-decoration:none" class="el-button el-button--success"
							v-if="isAuth('fangchanLiuyan','导入导出')" icon="el-icon-download"
							href="http://localhost:8080/fangdicanxiaoshouyingxiao/upload/fangchanLiuyanMuBan.xls">批量导入新房留言数据模板</a>
						&nbsp;
						<el-upload v-if="isAuth('fangchanLiuyan','导入导出')" style="display: inline-block"
							action="fangdicanxiaoshouyingxiao/file/upload" :on-success="fangchanLiuyanUploadSuccess"
							:on-error="fangchanLiuyanUploadError" :show-file-list=false>
							<el-button v-if="isAuth('fangchanLiuyan','导入导出')" type="success" icon="el-icon-upload2">
								批量导入新房留言数据</el-button>
						</el-upload>
						&nbsp;
						<!-- 导出excel -->
						<download-excel v-if="isAuth('fangchanLiuyan','导入导出')" style="display: inline-block"
							class="export-excel-wrapper" :data="dataList" :fields="json_fields"
							name="fangchanLiuyan.xls">
							<!-- 导出excel -->
			...
			<div class="table-content">
				<el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
					...
					v-if="isAuth('fangchanLiuyan','查看')" :data="dataList" v-loading="dataListLoading"
					@selection-change="selectionChangeHandler">
					<el-table-column v-if="contents.tableSelection" type="selection" header-align="center"
						align="center" width="50">
					</el-table-column>
					<el-table-column label="索引" v-if="contents.tableIndex" type="index" width="50" />
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="fangchanName"
						header-align="center" label="新房名称">
						<template slot-scope="scope">
							{{scope.row.fangchanName}}
						</template>
					</el-table-column>
					...
					<el-table-column :sortable="contents.tableSortable" 
					:align="contents.tableAlign" prop="fangchanLiuyanContent"
						header-align="center" label="留言内容">
						<template slot-scope="scope">
							{{scope.row.fangchanLiuyanContent}}
						</template>
					</el-table-column>
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="insertTime"
						header-align="center" label="留言时间">
						<template slot-scope="scope">
					...
					<el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="updateTime"
						header-align="center" label="回复时间">
						<template slot-scope="scope">
					...
					<el-table-column width="300" :align="contents.tableAlign" header-align="center" label="操作">
						<template slot-scope="scope">
							<el-button v-if="isAuth('fangchanLiuyan','查看')" type="success" icon="el-icon-tickets"
								size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
							<el-button v-if="isAuth('fangchanLiuyan','修改') " type="primary" icon="el-icon-edit"
								size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>

							<el-button v-if="isAuth('fangchanLiuyan','删除')" type="danger" icon="el-icon-delete"
	...
	export default {
		data() {
			return {
				...
				userId: "", //当前登录人的id
				//级联表下拉框搜索条件
				fangchanTypesSelectSearch: [],
				//当前表下拉框搜索条件
				form: {
					id: null,
					fangchanId: null,
					yonghuId: null,
					fangchanLiuyanContent: null,
					insertTime: null,
					replyContent: null,
					updateTime: null,
					createTime: null,
		...
		methods: {
			chartDialog() {
				...
				let params = {
					...
					thisTable: { //当前表
						tableName: 'fangchan_liuyan', //当前表表名,
						sumColum: 'fangchan_liuyan_number', //求和字段
						date: 'insert_time', //分组日期字段
						// string : 'fangchan_liuyan_name',//分组字符串字段
						// types : 'fangchan_liuyan_types',//分组下拉框字段
					},
					// joinTable : {//级联表（可以不存在）
				...
				//     var statistic = this.$echarts.init(document.getElementById("statistic"),'macarons');
				//     let params = {
				//         tableName: "fangchan_liuyan",
				//         groupColumn: "fangchan_liuyan_types",
				//     }
				//     this.$http({
			...
			getDataList() {
				...
				if (this.searchForm.fangchanName != '' && this.searchForm.fangchanName != undefined) {
					params['fangchanName'] = '%' + this.searchForm.fangchanName + '%'
				}

				if (this.searchForm.fangchanTypes != '' && this.searchForm.fangchanTypes != undefined) {
					params['fangchanTypes'] = this.searchForm.fangchanTypes
				}
				...
				params['fangchanLiuyanDelete'] = 1 // 逻辑删除字段 1 未删除 2 删除


				this.$http({
					url: "fangchanLiuyan/page",
					method: "get",
					params: params
				}).then(({
					data
				...
				//查询级联表搜索条件所有列表
				this.$http({
					url: "dictionary/page?dicCode=fangchan_types&page=1&limit=100",
					method: "get",
					page: 1,
					limit: 100,
				}).then(({
					...
					if (data && data.code === 0) {
						this.fangchanTypesSelectSearch = data.data.list;
					}
				});
				//查询当前表搜索条件所有列表
			},
			...
			// 导入功能上传文件成功后调用导入方法
			fangchanLiuyanUploadSuccess(data) {
				let _this = this;
				_this.$http({
					url: "fangchanLiuyan/batchInsert?fileName=" + data.file,
					method: "get"
				}).then(({
					data
				}) => {
			...
			// 导入功能上传文件失败后调用导入方法
			fangchanLiuyanUploadError(data) {
